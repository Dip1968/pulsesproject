...
function ApplyCareer() {
    ...
    const selectedCareer = careers.find((item)=> item.id === location.pathname.split('/')[2])
                     ...
                     <ul className="icon-list">
                        {
                            selectedCareer?.Expectations.map((description,index)=>{
                                return(
                                    <li>{description} <span style={{fontWeight:"bold"}}>{index===0 && 'IT organization'}</span></li>
                                )
                            })
                        }
                     </ul>
    ...
    <Form.Control
      ...
      {...register("ECTC", {
        ...
        validate: value => {
          ...
          const expectedCTC = parseFloat(value.replace(/\b(?!^\.?\d)\d*\.?\d*\b/g, '')); // Extract numeric value, handle lakh
                        ...
                        <>
                          ...
                          {showOtherInput && (
                            <Form.Control
                              ...
                              onChange={(e) => setNoticePeriodValue(e.target.value)}
